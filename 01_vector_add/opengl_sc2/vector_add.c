...
int Init ( SCContext *scContext )
{
   ...
   const char fShaderStr[] =  
      ...
      "}\n"
      "void main() {\n"
      "    highp vec2 uv = gl_FragCoord.xy / " xstr(F_SIZE) ".0;\n"
      "    highp float a, b;\n"
      "    decode_exact(a, textureUnit0, uv);\n"
      "    decode_exact(b, textureUnit1, uv);\n"
      "    highp float res = a + b;\n"
      "    encode_exact(res, gl_FragColor);\n"
      "}\n";
   ...
   for(int i=0; i<2; i++) {
        for(int r=0; r < SIZE; r++) {
            for(int c=0; c < SIZE; c++) {
                encode_float((float)c, &texture_d[(r * SIZE + c) * 4]);
            }
        }
...
void Draw ( SCContext *scContext )
{
   ...
   for(int j=0; j<SIZE; j++) {
       printf("%g ", decode_float(&out_data[(1*SIZE+j)*4]));
   }
   printf("\n");
   ...
   for (int i = 0; i < SIZE; i++) {
       for (int j = 0; j < SIZE; j++) {
           float gpu_val = decode_float(&out_data[(i * SIZE + j) * 4]);
           float cpu_ref = (float)j + (float)j;
           if (fabsf(gpu_val - cpu_ref) > 0.001f) {
...
int main ( int argc, char *argv[] )
{
   SCContext scContext;
   UserData  userData;
   out_data = malloc(4*SIZE*SIZE);
   texture_d = malloc(4*SIZE*SIZE);
   t_start = get_time();
   scInitContext ( &scContext );
   scContext.userData = &userData;
   scCreateWindow ( &scContext, "GPGPU Vector Addition SC2", SIZE, SIZE, SC_WINDOW_RGB|SC_WINDOW_ALPHA);
