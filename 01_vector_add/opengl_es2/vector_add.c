...
int Init(ESContext *esContext) {
    ...
    sprintf(shaderDefine, "#define FRAMEBUFFER_SIZE %.1f\n", (float)SIZE);
    ...
    for(int i=0; i<2; i++) {
        ...
        for(int row=0; row < SIZE; row++) {
            for(int col=0; col < SIZE; col++) {
                encode_float((float)col, &texture_data[(row * SIZE + col) * 4]);
            }
        }
...
void Draw(ESContext *esContext) {
    ...
    for(int i=0; i<SIZE; i++) {
        for(int j=0; j<SIZE; j++) {
            float gpu_val = decode_float(&output_data[(i*SIZE+j)*4]);
            // Reference: A[row][col]=col, B[row][col]=col -> C[row][col] = 2 * col
...
int main(int argc, char *argv[]) {
    ...
    esCreateWindow(&esContext, "GPGPU Vector Add", SIZE, SIZE, ES_WINDOW_RGB);
    if (Init(&esContext)) {
