...
private:
    ...
    void createInstance() {
        ...
        appInfo.pApplicationName = "MatMultOffscreen";
    ...
    void createOffscreenPipeline() {
        std::string shaderFilename = "mat_mult_f.frag";
        std::string fragShaderSource = readFileToString(shaderFilename);
        std::string vertShaderSource = readFileToString("mat_mult_v.vert");

        std::string sizeDefine = "#define MATRIX_SIZE " + std::to_string(SIZE) + ".0\n#define LOOP_SIZE " + std::to_string(SIZE) + "\n";
        
        std::cout << "Loading shader: " << shaderFilename << " (SIZE=" << SIZE << ")" << std::endl;
    ...
    void createTextureImage1() {
        ...
        for (int i = 0; i < SIZE; i++) {
            for (int j = 0; j < SIZE; j++) {
                ...
                encodeFloat((float)i, &pixels[(i * SIZE + j) * 4]);
            }
        }
    ...
    void verifyOutput() {
        ...
        for (int i = 0; i < SIZE; i++) {
            for (int j = 0; j < SIZE; j++) {
                ...
                if (std::abs(gpuVal - cpuRef) > 0.5f || std::isnan(gpuVal)) {
